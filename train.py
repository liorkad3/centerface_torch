...
train_transform = TrainAugmentation(args.input_size)
eval_transform = EvalAugmentation(args.input_size)

print("Prepare training dataset.")
train_dataset = WiderDataset(args.dataset_dir, mode='train')
train_loader = DataLoader(train_dataset, batch_size=args.batch_size,
             num_workers=args.num_workers, shuffle=True)
print("Prepare validation dataset.")
val_dataset = WiderDataset(args.dataset_dir, mode='val')
...
def train(loader, net, criterion, loss_weights, optimizer, device, debug_steps=100, epoch=0):
    ...
    for i, data in tqdm(enumerate(loader)):
        ...
        loss = c_loss + loss_weights['scale']*s_loss +
                 loss_weights['offset']*off_loss + loss_weights['landmark']*lms_loss
        loss.backward()
        optimizer.step()
...
loss_weights = {'offset':args.l_offset, 'scale':args.l_scale, 'landmark':args.l_landmark}
